...
class EventMatcher:
    ...
    def _load_normalization_map(self) -> Dict:
        ...
        try:
            # Look for normalization_map.json in the app directory
            app_dir = Path(__file__).parent.parent
            normalization_file = app_dir / "normalization_map.json"
            
            if normalization_file.exists():
                with open(normalization_file, "r", encoding="utf-8") as f:
                    return json.load(f)
            else:
                logger.warning(f"Normalization map not found at {normalization_file}")
                return {}
