...
class BaseScraper(ABC):
    ...
    async def initialize_browser(self, playwright_instance) -> None:
        """Initialize browser context and page."""
        try:
            # Launch browser from playwright instance
    ...
    def parse_decimal_odds(self, odds_text: str) -> Optional[float]:
        """Parse decimal odds from text."""
        return self.parse_odds_value(odds_text)

    def extract_line_from_market(self, market_name: str) -> tuple:
        """Extract line information from market name."""
        # Simplified implementation
        import re
        
        # Look for numeric values that might be lines
        line_match = re.search(r'(\d+\.?\d*)', market_name)
        if line_match:
            try:
                line = float(line_match.group(1))
                return market_name, line
            except ValueError:
                pass
        
        return market_name, None

    def validate_odds_data(self, odds_data: RawOddsData) -> bool:
        """Validate odds data before adding to results."""
        if not odds_data.event_name or not odds_data.outcome_name:
            return False
        
        if odds_data.odds < 1.01 or odds_data.odds > 1000:
            return False
            
        if not odds_data.url:
            return False
